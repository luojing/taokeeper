...
public class UtilSSH {
	...
	private final static String PRIVATEKEY = "none";
	 ...
	 * @since 1.0.0
	 */
	public static HostPerformanceEntity getHostPerformance( String ip, int port, String userName, String privatekey ) throws SSHException {
		...
		privatekey = StringUtil.defaultIfBlank(privatekey, PRIVATEKEY);
		Connection conn = null;
		try {
			conn = new Connection( ip, port );
			conn.connect( null, 10000, 10000 );
			boolean isAuthenticated = conn.authenticateWithPublicKey(userName, new File(privatekey), null);
			if ( isAuthenticated == false ) {
				throw new Exception( "SSH authentication failed with [ userName: " + userName + ", privatekey: " + privatekey + "]" );
			}
		...
		try {
			...
			while ( ( line = read.readLine() ) != null ) {
				...
				if ( 1 == lineNum ) {
					...
					systemPerformanceEntity.setMemoryUsage( df.format(memoryUsage * 100) + PERCENT );
				} else {
					continue;
				}
	 ...
	 * @param ip 主机ip
	 * @param username 用户名
	 * @param privatekey ssh key
	 * @param command 要执行的命令
	 */
	public static String execute( String ip, int port, String username, String privatekey, String command ) throws SSHException {
	 ...
	 * @throws SSHException
	 */
	public static String execute( String ip, String username, String privatekey, String command ) throws SSHException {
		return UtilSSH.execute( ip, PORT, username, privatekey, command );
	}
	 ...
	 * @param command 要执行的命令
	 */
	public static void executeNoOutPut( String ip, int port, String username, String privatekey, String command ) throws SSHException {
	 ...
	 * @param command 要执行的命令
	 */
	public static void executeNoOutPut( String ip, String username, String privatekey, String command ) throws SSHException {
		UtilSSH.executeNoOutPut( ip, PORT, username, privatekey, command );
	}
	 ...
	 */
	public static SSHResource executeWithoutHandleBufferedReader( String ip, int port, String username, String privatekey, String command ) throws SSHException {
		...
		try {
			...
			sshResource.conn.connect( null, 10000, 10000 );
			boolean isAuthenticated = sshResource.conn.authenticateWithPublicKey(username, new File(privatekey), null);
			if ( isAuthenticated == false ) {
				throw new Exception( "SSH authentication failed with [ userName: " + username + ", privatekey: " + privatekey + "]" );
			}
	 ...
	 * @return SSHResource 包含Connection, Session, BufferedReader
	 */
	public static SSHResource executeWithoutHandleBufferedReader( String ip, String username, String privatekey, String command ) throws SSHException {
		return UtilSSH.executeWithoutHandleBufferedReader( ip, PORT, username, privatekey, command );
	}
